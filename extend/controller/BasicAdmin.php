...
class BasicAdmin extends Controller
{
    ...
    protected function _form($dbQuery = null, $tplFile = '', $pkField = '', $where = [], $extendData = [])
    {
        ...
        if (false !== $this->_callback('_form_filter', $data, [])) {
            $result = DataService::saveGetId($db, $data, $pk, $where);
     ...
     * @throws \think\Exception
     */
    protected function _list($dbQuery = null, $isPage = true, $isFragment = false, $isDisplay = true, $total = false, $result = [])
    {
        $db = is_null($dbQuery) ? Db::name($this->table) : (is_string($dbQuery) ? Db::name($dbQuery) : $dbQuery);
        // 列表排序默认处理
        ...
        if ($isPage) {
            $base_uri = url('@admin');
            if ($isFragment) {
                $base_uri = url("@admin/index/fragment");
            }
            ...
            if (($totalNum = $page->total()) > 0) {
                ...
                foreach ([20, 50, 100, 150, 200] as $num) {
                    list($query['rows'], $query['page']) = [$num, '1'];
                    $url = $base_uri . '#' . $this->request->baseUrl() . '?' . http_build_query($query);
...
//                list($pattern, $replacement) = [['|href="(.*?)"|', '|pagination|'], ['data-open="$1"', 'pagination pull-right']];
                $info = '<ul class="pagination pull-right"><li class="disabled" data-tips-text="总计"><span>T' . $page->total() . '</span></li><li class="disabled" data-tips-text="本页"><span>C' . $page->count() . '</span></li>';
                $result['page'] = preg_replace(['|href="(.*?)"|', '|pagination|'], ['data-open="$1" href="javascript:void(0);"', 'pagination pull-right'], $page->render());
                $result['page'] = str_replace('<ul class="pagination pull-right">', $info, $result['page']);
                $result['pages'] = "<span class='pagination-trigger nowrap' style='position: relative;'>共 {$totalNum} 条记录，每页显示 <select data-auto-none>" . join('', $rowsHTML) . "</select> 条，共 " . ceil($totalNum / $rows) . " 页当前显示第 <select>" . join('', $pageHTML) . "</select> 页。</span>";
                list($result['total'], $result['list'], $result['page']) = [$totalNum, $page->all(), $result['page'],$result['pages']];
            } else {
