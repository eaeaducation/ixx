...
//                }
//            }
            $mime_file = getcwd() . '/config/mime.php';
            $mines = include($mime_file);
    ...
    public static function getFileUrl($filename, $storage = null)
    {
        ...
        switch (empty($storage) ? sysconf('storage_type') : $storage) {
            ...
            case 'minio':
                return self::getBaseUriMinio() . $filename;
        }
        throw new \think\Exception('未设置存储方式，无法获取到文件对应URL地址');
    }

    /**
     * 根据配置获取到Local文件上传目标地址
     * @return string
     */
    ...
    /**
     * 根据配置获取到 Minio Server 文件上传目标地址
     * @return string
     */
    public static function getUploadMinioUrl()
    {
        return url('@admin/plugs/upload_minio');
    }
     ...
     * @author Jasmine2
     */
    public static function getBaseUriMinio()
    {
        return sysconf('storage_minio_domain') . '/' . sysconf('storage_minio_bucket') . '/';
    }
    ...
    public static function hasFile($filename, $storage = null)
    {
        switch (empty($storage) ? sysconf('storage_type') : $storage) {
            ...
            case 'minio':
                $minioClient = new Client(sysconf('storage_minio_keyid'), sysconf('storage_minio_secret'), sysconf('storage_minio_endpoint'));
                $res = $minioClient->getObjectInfo(sysconf('storage_minio_bucket'), $filename);
                if ($res->code == 200) {
                    return true;
                }
        }
    ...
    public static function readFile($filename, $storage = null)
    {
        switch (empty($storage) ? sysconf('storage_type') : $storage) {
            ...
            case 'minio':
                $minioClient = new Client(sysconf('storage_minio_keyid'), sysconf('storage_minio_secret'), sysconf('storage_minio_endpoint'));
                $res = $minioClient->getObject(sysconf('storage_minio_bucket'), $filename);
                return $res->body;
            default:
     ...
     * @author Jasmine2
     */
    public static function minio($filename, $content)
    {
        $minioClient = new Client(sysconf('storage_minio_keyid'), sysconf('storage_minio_secret'), sysconf('storage_minio_endpoint'));
        $res = $minioClient->putObject(sysconf('storage_minio_bucket'), $filename, $content);
        if ($res->code == 200) {
            $url = sysconf('storage_minio_domain') . '/' . sysconf('storage_minio_bucket') . '/' . $filename;
