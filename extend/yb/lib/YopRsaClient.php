...
use yb\lib\Util\HttpRequest;
use yb\lib\Util\StringUtils;
use yb\lib\Util\HttpUtils;
use yb\lib\Util\Base64Url;
...
class YopRsaClient
{
    ...
    static public function handleRsaResult($request, $content)
    {
        ...
        \think\facade\Log::error($jsoncontent);
        $response->result = $jsoncontent->result;
        if(property_exists($jsoncontent,'state')){
