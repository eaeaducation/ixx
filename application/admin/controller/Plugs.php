...
class Plugs extends BasicAdmin
{
    ...
    public function upfile()
    {
        ...
        if (!in_array($uptype, ['local', 'qiniu', 'oss', 'minio'])) {
    ...
    public function upload()
    {
        ...
        // 文件上传处理
        if (($info = $file->move("static/upload/" . date('ymd'), "{$names}.{$ext}", true))) {
    ...
    public function upload_minio()
    {
        ...
        if (FileService::save($filename, $fileContent, 'minio')) {
            if (($site_url = FileService::getFileUrl($filename, 'minio'))) {
                return json(['data' => ['site_url' => $site_url], 'code' => 'SUCCESS', 'msg' => '文件上传成功']);
            }
        }
    }
    ...
    public function upstate()
    {
        ...
        $filename = date('ymd') . '/' . $post['md5'] . '.' . strtolower(pathinfo($post['filename'], 4));
        // 检查文件是否已上传
        ...
        switch (strtolower($post['uptype'])) {
            ...
            case 'minio':
                $config['uptype'] = 'minio';
                $config['server'] = FileService::getUploadMinioUrl();
