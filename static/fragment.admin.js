...
$(function () {
    ...
    $.msg = new function () {
        ...
        this.auto = function (data, time) {
            if (parseInt(data.code) === 1 && data.data.close_fragment) {
    ...
    /*! 注册 data-push-ids 事件行为, 向父级页面传递ID */
    $body.on('click', '[data-push-ids]', function () {
        var id = $(this).attr('data-push-ids') || (function () {
            var data = [];
            return $($(this).attr('data-list-target') || 'input.list-check-box').map(function () {
                (this.checked) && data.push(this.value);
            }), data.join(',');
        ...
        var msg = $(this).attr('data-title') || false;
        var callback = $(this).attr('data-callback') || 'callback';
        if (msg) {
            $.msg.confirm(msg, function () {
                parent[callback].call(parent, id);
