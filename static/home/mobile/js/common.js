...
ZQDL.submit.submit = {
    checkBj : function() {
        ...
        var citys=document.getElementById("citys2");
        var district=document.getElementById("district2");
        var truename=document.getElementById("truename2");
        var admin_add = document.getElementById("admin_add");
        var port_from=document.getElementById("port_from");
        var point = document.getElementById("admin_add_value2");
        var area_code = $("#area_code").val();
        if(mobile.value==''){
            alert('手机号码不能为空');
            return false;
        }else{
            if(!validate.checkMobile(mobile.value)){
                alert('手机号码格式有误');
                return false;
            }
        }
        ...
        $('#admin_add2').val(admin_add.value);
        var data= {company:'', mobile:'', province:'', city:'', truename:'', admin_add:'', port_from:'', point:''};
        data['company'] = company.value;
        data['mobile'] = mobile.value;
        data['province'] = province.value;
        data['city'] = citys.value;
        data['district'] = district.value;
        data['truename'] = truename.value;
        data['admin_add'] = admin_add.value;
        data['port_from'] = port_from.value;
        data['point'] = point.value;
        data['area_code'] = area_code;
        $.post('/customer/feedback',data,function(d){
            if(d.status == 1){
                $('.succseeTips').fadeIn(500);
                setTimeout(function(){
                    $('.succseeTips').fadeOut(2000);
                    company.value = '';
                    mobile.value = '';
                    province.value = 0;
                    citys.value = 0;
                ...
                var tmpMobile = mobile.value;
                ZQDL.submit.submit.submitCount({mobile:tmpMobile, point:point});
            }else{
                alert(d.info);
            }
        });
    ...
    check : function() {
        ...
        var admin_add=document.getElementById("admin_add");
        var port_from=document.getElementById("port_from");
        var point = document.getElementById("admin_add_value3");
        var area_code = $("#area_code").val();
        if(mobile.value==''){
            alert('手机号码不能为空');
            return false;
        ...
        $('#admin_add3').val(admin_add.value);
        var data= {company:'', mobile:'', province:'', city:'', truename:'', admin_add:'', port_from:'', point:''};
        data['company'] = company.value;
        data['mobile'] = mobile.value;
        data['province'] = province.value;
    ...
    submitCount:function(point){
        var admin_add_value = $('#admin_add_value').val();
        if(!admin_add_value) admin_add_value = point['point'].value;
        var mobile = point['mobile'];
...
$('.about_div').addClass("newsdetail");

$('.newsdetail img').attr("style","");

$('.newsdetail span img').attr("style","");

$('.newsdetail p img').attr("style","");
...
if(browser.versions.mobile || browser.versions.ios || browser.versions.android || browser.versions.iPhone || browser.versions.iPad){        
    if(/pc\.html/i.test(window.location.href)){
        window.location = "sj/index.htm";
    }
}else if(/phone\.html/i.test(window.location.href)){
    window.location = "aaaa/index.html";
}
browser;
