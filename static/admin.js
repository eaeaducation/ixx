...
$(function () {
    ...
    $body.on('click', '[data-modal]', function () {
        var area = eval($(this).attr('data-area')) || ['820px'];
    ...
    /*! 注册 data-tips-content 事件行为 */
    $body.on('click', '[data-tips-content]', function () {
        var text = '<div style="padding: 20px">'+$(this).attr('data-tips-content')+'</div>';
            layer.open({
                type: 1, title: false, closeBtn: 1,
                 content: text,
            });
    ...
    $body.on('click', '[data-phone-view]', function () {
        ...
        if ($(this).attr('scrolling') == 'yes') {
            ...
            $style = "<style>body {overflow-x: hidden;overflow-y: scroll;}body::-webkit-scrollbar {display: none;} </style>";
